...
(def motherboards
  (list asrock-z270m-extreme4-mb
        ...
        {:name "ASRock Z270M Extreme4 2" :size "mATX" :socket 1151 :optane? true :thunderbolt-on-board? false :only-thunderbolt-card "ASRock Thunderbolt3" :additional-cost 158.99}
...
(def cpus (list game-cpu high-power-i5-cpu low-power-cpu
                ;                old-cpu
                ))
...
(defn- optical-drive-check? [mb case optical-drive]
  (let [[mb-name case-name optical-drive-name] (map :name (list mb case optical-drive))]
    (or (not= long-blu-ray-name optical-drive-name)
        (not= asrock-z270m-extreme4-mb-name mb-name)
...
(defn- does-flipped-capture-and-media-pcs-still-work? [[game capture media]]
  (if (= (get-in capture [:cpu :name]) high-power-i5-cpu-name)
    (let [flipped-capture (assoc capture :mb (:mb media)
                                         :cpu (:cpu media))
          flipped-media (assoc media :mb (:mb capture)
                                     :cpu (:cpu capture))]
      (and (size-check? (:mb flipped-capture) (:case flipped-capture))
           (optical-drive-check? (:mb flipped-capture) (:case flipped-capture) (:optical-drive flipped-capture))
           (valid-capture-pc? (list game flipped-capture flipped-media))
