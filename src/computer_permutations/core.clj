...
(def asrock-z270m-extreme4-mb-2 {:name "ASRock Z270M Extreme4 2" :size "mATX" :socket 1151 :optane? true :thunderbolt-on-board? false :only-thunderbolt-card "ASRock Thunderbolt3" :additional-cost 158.99})
...
(defn- does-flipped-capture-and-media-pcs-still-work? [[game capture media]]
  (if (= (get-in capture [:cpu :name]) high-power-i5-cpu-name)
    (let [flipped-capture (flip-components capture media)
      ...
      (seq (filter true? (map is-flipped-capture-valid? (list flipped-capture flipped-capture2)))))
    true))
